...
-- Trigger: booking created/updated -> notify both parties
...
-- Trigger: new message -> notify the other party (skip system messages)
create or replace function public.on_message_notify()
returns trigger language plpgsql security definer set search_path = public as $$
declare _customer uuid; _provider uuid; _recipient uuid; _preview text;
begin
  if new.kind = 'system' then return new; end if;
  select customer_id into _customer from public.job_requests where id = new.job_id;
  select provider_id into _provider from public.quotes where job_id = new.job_id and provider_id = new.sender_id limit 1;
  if new.sender_id = _customer then
    -- recipient is the provider on this thread; pick from any quote/booking
